...
expName = u'filevich_et_al'  # from the Builder filename that created this script
...
intro_txt = visual.TextStim(win=win, name='intro_txt',
    text=u'Hello! The experiment starts after a key press.',
    font=u'Open Sans',
...
trial_finished_msg = visual.TextStim(win=win, name='trial_finished_msg',
    text=u'Press any key for the next trial.',
    font=u'Open Sans',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color=u'white', colorSpace='rgb', opacity=1,
    depth=-6.0);
...
for thisMain_trial in main_trials:
    ...
    while continueRoutine:
        ...
        if t >= 0.0 and fixation_1.status == NOT_STARTED:
            # keep track of start time/frame for later
            objColorRand = random()
            if objColorRand>=0.0 and objColorRand<0.2:
                circle_2.lineColor='Red'
                circle_2.fillColor='Red'
            elif objColorRand>=0.2 and objColorRand<0.4:
                circle_2.lineColor='Blue'
                circle_2.fillColor='Blue'
            elif objColorRand>=0.4 and objColorRand<0.6:
                circle_2.lineColor='Yellow'
                circle_2.fillColor='Yellow'
            elif objColorRand>=0.6 and objColorRand<0.8:
                circle_2.lineColor='Purple'
                circle_2.fillColor='Purple'
            elif objColorRand>=0.8:
                circle_2.lineColor='Yellow'
                circle_2.fillColor='Yellow'
            fixation_1.tStart = t
        ...
        # *fixation_3* updates
        if t >= 1.75 and fixation_3.status == NOT_STARTED:
            # keep track of start time/frame for later
            win.color = 'Black'
            fixation_3.tStart = t
            fixation_3.frameNStart = frameN  # exact frame index
            fixation_3.setAutoDraw(True)
        frameRemains = 1.75 + 0.8- win.monitorFramePeriod * 0.75  # most of one frame period left
        ...
        if t >= 2.55 and fixation_3.status == FINISHED and trial_finished_msg.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        # *key_resp_5* updates
        if t >= 2.55 and fixation_3.status == FINISHED and key_resp_5.status == NOT_STARTED:
            # keep track of start time/frame for later
            key_resp_5.tStart = t
